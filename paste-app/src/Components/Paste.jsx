...
import { deletePaste, fetchPastes } from '../redux/pasteSlice';
import { authAPI } from '../utils/api';
import { toast } from 'sonner';
import { Link, useNavigate } from "react-router-dom";
import { Edit, Eye, Trash2, Copy, Share2, Search, Calendar, Clock, BarChart } from 'lucide-react';
...
const Paste = () => {
    ...
    function handleShare(paste) {
        // Use uniqueId for shareable links
        const shareUrl = `${window.location.origin}/pastes/${paste.uniqueId || paste._id}`;
                                                    ...
                                                    <div className="flex items-center gap-3 text-sm text-muted-foreground">
                                                        ...
                                                        {paste.expiresAt && (
                                                            <div className="flex items-center gap-1">
                                                                ...
                                                                <span>Expires: {new Date(paste.expiresAt).toLocaleDateString('en-US', {
                                                                    month: 'short',
                                                                    day: 'numeric',
                                                                    hour: '2-digit',
                                                                    minute: '2-digit'
                                                        ...
                                                        {paste.isDraft && (
                                                            <span className="px-2 py-0.5 bg-yellow-500/20 text-yellow-600 dark:text-yellow-400 rounded text-xs">Draft</span>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
